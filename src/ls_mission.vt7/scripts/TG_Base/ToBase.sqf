h = [] spawn {
	...
	_ctrlEdit ctrlAddEventHandler ["KeyUp", {params ["_ctrl", "_key"];
		...
		if (_isValid) then {
			...
			then {player setposatl [getpos tg_base select 0, getpos tg_base select 1, getposatl tg_base select 2]; titleText ["Вы сели в машину и доехали до лагеря ТГ", "BLACK FADED", 0.2];} 
