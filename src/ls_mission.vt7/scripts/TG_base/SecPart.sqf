h = [] spawn {
	...
	_ctrlEdit ctrlAddEventHandler ["KeyUp", {params ["_ctrl", "_key"];
		...
		if (_isValid) then {
			...
			then {player setposatl [getpos sec_in select 0, getpos sec_in select 1, getposatl sec_in select 2]; titleText ["После введения пароля дверь открылась вы зашли и очутились на распутье", "BLACK FADED", 0.2];} 
