h = [] spawn {
	...
	_ctrlEdit ctrlAddEventHandler ["KeyUp", {params ["_ctrl", "_key"];
		...
		if (_isValid) then {
			...
			if (_number isEqualTo 55027) 
			then {player setposatl [getpos main_in select 0, getpos main_in select 1, getposatl main_in select 2]; titleText ["После введения пароля ворота отворились, вы долго шли по тёмным коридорам, когда вы увидели свет, вы поняли что пришли", "BLACK FADED", 0.2];} 
