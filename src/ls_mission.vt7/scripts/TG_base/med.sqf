h = [] spawn {
	...
	_ctrlEdit ctrlAddEventHandler ["KeyUp", {params ["_ctrl", "_key"];
		...
		if (_isValid) then {
			...
			if (_number isEqualTo 77129) 
			then {player setposatl [getpos med_in select 0, getpos med_in select 1, getposatl med_in select 2]; titleText ["После введения пароля дверь открылась вы вошли в неё и оказались в мед.блоке", "BLACK FADED", 0.2];} 
